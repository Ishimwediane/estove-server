...
const char* latestDataEndpoint = "/api/stove-data/latest";
...
const unsigned long COMMAND_CHECK_INTERVAL = 3000; // Check for commands every 3 seconds
bool lastWebCookingState = false;
bool lastWebManualState = false;
unsigned long lastWebTimeLeft = 0;
...
void checkWebCommands() {
  ...
  HTTPClient http;
  String url = String(serverUrl) + String(latestDataEndpoint);
  ...
  if (httpResponseCode > 0) {
    String response = http.getString();
    Serial.println("📥 Received server data: " + response);
    
    // Parse the response to check for commands
    // Look for cooking state changes
    if (response.indexOf("\"cooking\":true") != -1 && !lastWebCookingState) {
      // Web started cooking
      Serial.println("🌐 Web command: Start cooking");
      cooking = true;
      manualMode = false;
      manualStartTime = 0;
      
      // Extract timeLeft from response
      int timeLeftIndex = response.indexOf("\"timeLeft\":");
      if (timeLeftIndex != -1) {
        int startIndex = timeLeftIndex + 11;
        int endIndex = response.indexOf(",", startIndex);
        if (endIndex == -1) endIndex = response.indexOf("}", startIndex);
        if (endIndex != -1) {
          String timeLeftStr = response.substring(startIndex, endIndex);
          unsigned long timeLeft = timeLeftStr.toInt();
          cookingEndTime = millis() + (timeLeft * 1000);
          lastWebTimeLeft = timeLeft;
          Serial.println("⏰ Cooking time set to: " + String(timeLeft) + " seconds");
        }
      }
      
      digitalWrite(relayPin, LOW); // Turn ON relay
      tempControlEnabled = true;
      manualOverrideDisabled = true; // Disable manual override temporarily
      
    } else if (response.indexOf("\"cooking\":false") != -1 && lastWebCookingState) {
      // Web stopped cooking
      Serial.println("🌐 Web command: Stop cooking");
      cooking = false;
      manualMode = false;
      cookingEndTime = 0;
      manualStartTime = 0;
      ...
      manualOverrideDisabled = false; // Re-enable manual override
      
    } else if (response.indexOf("\"manualMode\":true") != -1 && !lastWebManualState) {
      // Web activated manual mode
    ...
    } else if (response.indexOf("\"manualMode\":false") != -1 && lastWebManualState) {
      // Web deactivated manual mode
    ...
    lastWebCookingState = (response.indexOf("\"cooking\":true") != -1);
    lastWebManualState = (response.indexOf("\"manualMode\":true") != -1);
...
void loop() {
  ...
  // Manual switch control (only if not disabled by web commands)
